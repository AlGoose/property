<template>
    <v-container fluid>
        ...
        <v-form ref="form" v-model="valid" lazy-validation>
            ...
            <v-row>
                <v-col cols="12" md="6">
                    <v-text-field v-model="dealer.inn.data" v-mask="dealer.inn.mask" label="ИНН"
                                  outlined></v-text-field>
                </v-col>

                <v-col cols="12" md="6">
                    <v-text-field v-model="dealer.kpp.data" v-mask="dealer.kpp.mask" label="КПП"
                                  outlined></v-text-field>
                </v-col>
            ...
            <v-card v-for="(group, key) in form" :key="key">
                <v-card-title><h3>{{group.title}}</h3>
                </v-card-title>
                <v-card-text>
                    <v-row>
                        <v-col v-for="item in group.items" :key="item.id"
                            ...
                            <template v-else>
                                ...
                                >
                                    <template v-slot:activator="{ on }">
                                        <v-text-field v-model="item.data" :label="item.label" readonly outlined
                                                      v-on="on"></v-text-field>
                                    </template>
                                    <v-date-picker v-model="item.data" no-title scrollable locale="Rus">
                                        <v-spacer></v-spacer>
                                        <v-btn text color="primary" @click="menu = false">Отмена</v-btn>
                                        <v-btn text color="primary" @click="$refs.menu[0].save(item.data)">Выбрать
                                        </v-btn>
                                    </v-date-picker>
                                </v-menu>
                            </template>
